...
# check visually in IGV whether the input coverage is even across the genome
...
do
	...
	bamCompare --operation ratio --binSize 1 -p "${CPU}" --scaleFactorsMethod readCount \ 
	  -b1 "${ip_file}" \
	  -b2 "${input_file}" \
	  -o "${coverage_dir}${bam_files[${i}+${sample_count}-1]}.input-normalized.bw"
done
```
