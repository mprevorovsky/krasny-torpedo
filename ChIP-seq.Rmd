...
plot(counts.len.norm[, 'ko.chip'] + pseudocount, counts.len.norm[, 'ko.rna'] + pseudocount,
     main = '',
     xlab = '',
     ylab = '',
     log = 'xy', xlim = c(1, limits[2]), ylim = c(0.01, limits[2]),
     type = 'n')
abline(v = c(1, 10, 100, 1000, 10000), h = c(0.01, 1, 100, 10000), col = 'gray')
par(new = TRUE)
plot(counts.len.norm[, 'ko.chip'] + pseudocount, counts.len.norm[, 'ko.rna'] + pseudocount,
     main = 'rnjA KO\npooled 3 replicates\n(coloured by expression in WT)',
     xlab = 'RNAP occupancy at genes [RPKM]',
     ylab = 'gene RNA abundance [RPKM]',
     log = 'xy', xlim = c(1, limits[2]), ylim = c(0.01, limits[2]), pch = 20,
     col = rbPal(100)[as.numeric(cut(log2(counts.len.norm[, 'wt.rna'] + 0.01), breaks = 100))])

plot(counts.len.norm[, 'wt.chip'] + pseudocount, counts.len.norm[, 'wt.rna'] + pseudocount,
     main = '',
     xlab = '',
     ylab = '',
     log = 'xy', xlim = c(1, limits[2]), ylim = c(0.01, limits[2]),
